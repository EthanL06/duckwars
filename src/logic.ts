...
type GameActions = {
  ...
  clearLastEvent: () => void;
  moveShip: (params: { ship: Ship; newCell: Cell }) => void;
...
const isValidPosition = (board: Board, ship: Ship, newPosition: Position) => {
...
const moveShip = (board: Board, ship: Ship, newCell: Cell) => {
...
Rune.initLogic({
  ...
  actions: {
    ...
    moveShip: (params, { game, playerId }) => {
      const { ship, newCell } = params;

      const board = game.boards[playerId];

      const newBoard = moveShip(board, ship, newCell);
      game.boards[playerId] = newBoard;
      return;
    },

    rotateShip: (selectedCell, { game, playerId }) => {
      const board = game.boards[playerId];
      ...
      //   ship: newShip,
      //   newCell: ship.startingPosition,
      // });

      const newBoard = moveShip(board, newShip, ship.startingPosition);
      game.boards[playerId] = newBoard;
